  ...
  try {
    ...
    return data.skills.map((skill) => {
      ...
      return {
        ...
        url: skill.topSource
          ? `https://skills.sh/${skill.topSource}/${skill.id}`
          : `https://skills.sh/${skill.id}`,
 ...
 */
export async function fetchPopularSkills(limit = 50, sort: 'all-time' | 'weekly' | 'monthly' = 'all-time'): Promise<Skill[]> {
  try {
